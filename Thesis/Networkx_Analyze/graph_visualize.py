...
def kempe_chain(perioding_exams,P,G):
    exam=random.choice([exam for exam in perioding_exams])
    neighbor=random.choice(list(G.neighbors(exam)))
    period1=perioding_exams[exam]
    period2=perioding_exams[neighbor]
    versus_period={
        period1:period2,
        period2:period1
    }   
    ...
    while not kc.empty():
        current_exam=kc.get()
        current_period=perioding_exams[current_exam]
        new_period=versus_period[current_period]
        moves[current_exam]=new_period
        for neighbor in list(G.neighbors(current_exam)):
            if neighbor in moves: continue
            if perioding_exams[neighbor]==new_period:
                kc.put(neighbor)
    return True,moves
...
def main_5():
    ...
    for index,node in enumerate(pdot.get_nodes()):
        ...
        node.set_fontcolor('black')
        node.set_color(color_map[index])
    pdot.write_png('kempe_chain_1.png')
    check,moves=kempe_chain(periods,P,G)
    while not check:
        check,moves=kempe_chain(periods,P,G)
    counter=2
    for exam,period in moves.items():
        ...
        for index,node in enumerate(pdot.get_nodes()):
            ...
            node.set_color(color_map[index])
        if edge_1==-1:
            edge_1=node.get_name()
        else:
            edge_2=node.get_name()
        dot_edge=pydot.Edge(edge_1,edge_2)
        if edge_1!=-1 and edge_2!=-1:
            dot_edge.color='blue'
        if counter%2==0:
            pdot.write_png(f'kempe_chain_{counter}.png')
        counter+=1
