...
class Problem:
    ...
    def save_solution_to_file(self):
        with open(os.path.join('','Solutions',self.pid+'.sol'),'w') as WF:
            for exam,period in self.s_periods.items():
                WF.write(f'{exam} {period};')
            WF.write('\n')
            WF.write(f'{self.compute_cost()}({self.compute_normalized_cost()})')
    
    def append_to_cost_csv(self,cost_file):
        with open(cost_file,'a') as AF:
            AF.write(f'{self.pid},{self.compute_cost}({self.compute_normalized_cost()})')
    ...
    def polish(self,best,best_sol,execution_time):
        ...
        while True:
            ...
            for exam in exams:
                ...
                for neighbor in list(self.G.neighbors(exam)):
                    period1=self.solutions[neighbor]
                    if self.can_be_moved(exam,period1,None,{neighbor}) and self.can_be_moved(neighbor,period2,None,{exam}):
                        moves={
                            exam:period1,
                            neighbor:period2
                        }
            ...
            for exam in exams:
                for neighbor in list(self.G.neighbors(exam)):
                    period1=self.solutions[neighbor]
                    if self.can_be_moved(exam,period1,None,{neighbor}):
                        for p in range(self.P):
                            if p==period1 or p==self.solutions[exam]: continue
...
def simulated_annealing(exec_time,dataset,storage_file=None):
    logging.basicConfig(level=logging.INFO,format='%(asctime)s \t %(message)s')
    temp=1000
    start_temp=1000
    alpha=0.9999
    ...
    s.save_solution_to_file()
    if storage_file!=None:
        s.append_to_cost_csv(storage_file)
...
def hill_climbing(dataset,exec_time,storage_file=None):
    logging.basicConfig(level=logging.INFO,format='%(asctime)s\t%(message)s')
    ...
    if storage_file!=None:
        sol.append_to_cost_csv(storage_file)
...
# def main_2():
#     all_ds=dict()
#     all_ds.update(carterhandler.datasets)
#     all_ds.update(itchandler.datasets)
#     for file in os.listdir(os.path.join('','Solutions')):
#         data=file.split('.')
#         if data[0] in all_ds:
#             all_ds.pop(data[0])
#     for dataset in all_ds:
#         print('#Dataset:{}'.format(dataset))
#         simulated_annealing(500,dataset)
...
def carter_sa_solution():
    data_path=os.path.join('','simulated_annealing')
    name_of_info_file=os.path.join(data_path,f'{platform.uname().machine}_sa_{exec_time}.csv')
    for dataset in carterhandler.datasets:
        print('#Dataset:{}'.format(dataset))
        simulated_annealing(exec_time,dataset,name_of_info_file)

def carter_hc_solution():
    data_path=os.path.join('','hill_climbing')
    name_of_info_file=os.path.join(data_path,f'{platform.uname().machine}_sa_{exec_time}.csv')
    for dataset in carterhandler.datasets:
        print('#Dataset:{}'.format(dataset))
        hill_climbing(exec_time,dataset,name_of_info_file)

def itc_sa_solution():
    data_path=os.path.join('','simulated_annealing')
    name_of_info_file=os.path.join(data_path,f'{platform.uname().machine}_sa_{exec_time}.csv')
    for dataset in itchandler.datasets:
        print('#Dataset:{}'.format(dataset))
        simulated_annealing(exec_time,dataset)
